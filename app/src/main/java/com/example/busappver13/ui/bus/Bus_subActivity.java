...
import com.example.busappver13.ui.home.HomeBusItems;
...
public class Bus_subActivity extends AppCompatActivity {
    ...
    class BusAsync2 extends AsyncTask<String, String, ArrayList<BusItems>> {
        ...
        protected void onPostExecute(ArrayList<BusItems> BusItems) {
            ...
            if (adapter == null) {
                adapter = new ListOfBusLocationsByRouteViewModelAdapter(Bus_subActivity.this,R.layout.each_bus_item, BusItems, mySubListView);
    ...
    private void addHomeFragmentList(){

        mySubListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {


                //alertDialog 생성
                AlertDialog.Builder dialog = new AlertDialog.Builder(Bus_subActivity.this);

                dialog.setPositiveButton("ok", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        BusItems busItems=(BusItems)adapter.getItem(position);
                        HomeBusItems homeBusItems=new HomeBusItems();

                        homeBusItems.setTotalCount(parser.getBi().getTotalCount());
                        homeBusItems.setNodeNm(busItems.getNodeNm());
                        homeBusItems.setVehicleNo(busItems.getVehicleNo());
                        homeBusItems.setSaved(true);

                        Intent intent=new Intent(Bus_subActivity.this,BusRequestActivity.class);
                        intent.putExtra("totalCount",homeBusItems.getTotalCount());
                        intent.putExtra("nodeNm",homeBusItems.getNodeNm());
                        intent.putExtra("vehicleNo",homeBusItems.getVehicleNo());
                        startActivity(intent);

                        Toast.makeText(getApplicationContext(),homeBusItems.getNodeNm()+", "+homeBusItems.getTotalCount()+", "+homeBusItems.getVehicleNo()+", "+homeBusItems.isSaved(), Toast.LENGTH_SHORT).show();
                    }
                });
